...
<p align="center">
  <strong>一條管線。相同的護欄。無論是你的團隊還是 AI Agent 在寫程式碼。</strong>
</p>
...
你沒有程式碼品質問題。你有的是**一致性問題。**

十個工程師寫了十種不同的 AJAX 呼叫。十個 AI Agent 生成十種不同的 fetch 模式。有人驗證表單，有人不驗證。有人顯示 loading 遮罩，有人忘記。有人優雅地處理錯誤，有人默默吞掉。

Code review 能抓到一部分。**架構能抓到全部。**

| 真正的問題 | 實際發生的事 |
|---|---|
| 「每個開發者的 AJAX 寫法都不同。」 | 驗證被跳過。Loading 動畫不一致。錯誤處理像擲硬幣。 |
| 「AI Agent 產出冗長、重複的程式碼。」 | 每個 Agent 從頭展開 `fetch` + 驗證 + 錯誤處理 + loading，把 context token 燒在樣板程式碼上。 |
| 「新成員打破慣例。」 | 他們根本不知道有慣例——那是口耳相傳的部落知識，不是被強制執行的基礎設施。 |
| 「我們無法判斷 Agent 是否漏了什麼。」 | 你必須審查每個生成的函式。大規模下，這是不可能的。 |

**解決方案不是更多的 code review，而是讓錯誤的程式碼不可能被寫出來。**

---

## 2. 運作方式

TS-Util 將 AJAX、VIEW、驗證、格式化和訊息整合進**一條強制執行的管線**。當任何人——人類或 AI——呼叫 `AJAX.request()` 時，以下步驟自動發生：
...
同樣的原則適用於 `VIEW.load()`——每一段動態載入的 HTML 片段都會自動經過約束綁定、輸入格式化和自訂 hook 執行。不需要手動初始化。不會「忘記」在新內容上設定驗證。
...
當多個 AI Agent 共同產出程式碼時，這層抽象變得更加關鍵：

- **Token 精簡**：Agent 只需要產出 `AJAX.request({ url, form })` 一行，而非每次重複展開 `fetch` + 驗證 + 錯誤處理 + loading 的完整邏輯。Context window 是 AI 最寶貴的資源；節省 token 就是維持品質。
- **行為可預測**：不同 Agent 產出的程式碼會通過同一條管線，確保結果一致。你不需要逐一審查每個 Agent 是否正確實作了 loading 遮罩。
- **護欄效果**：抽象層本身就是一道護欄。Agent 無法「忘記」驗證表單，因為 `AJAX.request()` 會自動執行。規範不靠記憶維護，而靠架構強制。

### 核心洞察

> **紀律不是「記得做對的事」。紀律是讓對的事成為唯一能發生的事。**
>
> 這就是 TS-Util 做的——為你今天的團隊，也為明天將寫出大部分程式碼的 AI Agent。
...
展示頁讓你點擊體驗 Events、AJAX、Validation、Formatting、MSG 對話框、VIEW 注入和工具函式——程式碼片段旁邊就有即時結果。
...
所有模組透過型別化的 `EventEmitter` 溝通——沒有模組直接匯入另一個模組。這使每個元件都可以獨立測試和替換。
...
這個程式庫是一個適合教學的程式碼庫。每個模組都實作了一個具名的 GoF 設計模式：
