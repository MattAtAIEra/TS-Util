# Why Wrap AJAX and VIEW?

> Available in: [繁體中文](#繁體中文) · [English](#english) · [日本語](#日本語) · [한국어](#한국어) · [Español](#español) · [Deutsch](#deutsch)
...
Every frontend project eventually hits the same problem: ten engineers write ten different ways to make an AJAX call. Some use `fetch`, some use `XMLHttpRequest`, some wrap `axios` their own way. Should you validate the form before sending? Show a loading overlay? How should errors be handled? Everyone has a different answer, and code review can only do so much.

**The core idea behind TS-Util is: encode decisions into infrastructure.**

When you call `AJAX.request()`, the following happens automatically — form validation, loading state management, error event broadcasting, and data serialization. You don't need to remember these steps, and you can't skip any of them.

Similarly, `VIEW.load()` ensures every dynamically loaded HTML fragment goes through the full initialization pipeline: constraint binding, input formatting, and custom hook execution. No matter who wrote the code, no matter whether the HTML was present at page load or fetched five minutes later via AJAX, the behavior is identical.
